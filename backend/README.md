...
> **Aborting a run can't cancel an in-flight sandbox creation.** `createWorkItemResolveSandbox` doesn't take the run's `AbortSignal` because the OpenSandbox SDK's `SandboxCreateOptions` has no signal/cancellation field at all — `Sandbox.create()` runs to completion (or its own `requestTimeoutSeconds`, currently 5 minutes for cold image pulls) regardless of abort. An aborted run sitting in sandbox creation only actually stops once that call resolves. Fixing this for real needs either an upstream SDK change or racing the create promise against the signal so the run at least stops waiting (and tears down whatever sandbox eventually comes back) — not yet implemented.
