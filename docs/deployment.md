...
**Required GitLab CI/CD variables** (see [`.gitlab-ci.yml`](../.gitlab-ci.yml) and the root [`README.md`](../README.md#cicd) for the full table): `SSH_PRIVATE_KEY`, `SSH_HOST`, `SSH_USER`, `SSH_KNOWN_HOSTS`. The deploy path itself is **not** a CI variable — it's baked into the `command=` restriction above, so changing it means editing `authorized_keys` on the server, not GitLab's CI/CD settings.
