...
Don't add a bare-metal Python venv under `/opt/sglang`. We used one (`sglang-env/`) during initial evaluation to debug several kernel/CUDA-toolkit issues (see [Known issues](#known-issues-and-history)), then removed it once the Docker stack was confirmed working end-to-end — if you find yourself reaching for one again, treat it as a fresh evaluation artifact, not something to merge back into the production path.
